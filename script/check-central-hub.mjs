...
requireMatch(html, /TARGET @ PINNED CUT[\s\S]*Constellation-native[\s\S]*SUBJECT-SCOPED DISPOSITIONS[\s\S]*BYOM and Electron retired[\s\S]*Electron is removed in the current Hive IDE build[\s\S]*tester\.5 predates the removal[\s\S]*Docker client requirements are not adjudicated[\s\S]*NO-LLM CLAIM · HOLD[\s\S]*centralized AI-as-user traffic until H10/i, "subject-scoped legacy and no-LLM HOLD translation");
